...
function History() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const { data: historiesDetail } =
          await hiddenPriceService.actGetHiddenPriceHistoryDetailById(
            historyId,
            {
              pageSize,
              pageIndex: pageDetail,
            }
          );

        setHistoriesDetail(historiesDetail);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
    ...
    (async () => {
      try {
        ...
        const { data: histories } =
          await hiddenPriceService.actGetHiddenPriceHistoryBySkd(sku, {
            pageSize,
            pageIndex: page,
          });
  ...
  const rendersColumn = (column) => {
    return column.map(({ name, class: classes = '' }) => (
      ...
      <tr key={id} className="bg-white border-b">
        ...
        <td className="px-2 py-2 text-right">
          ...
          >
            ...
            >
              <path
                d="M9.42188 3.9375L14.4844 9L9.42188 14.0625"
                stroke="#0049C6"
                strokeWidth="2.25"
                strokeLinecap="round"
  ...
  const renderHistoriesDetail = () => {
    if (emptyArray(historiesDetail)) {
      return null;
    }

    return historiesDetail.map(({ id, price, createAt, type, point }) => (
      <tr key={id} className="bg-white border-b">
        <td className="px-2 py-2">{id}</td>
        <td className="px-2 py-2">{type === 1 ? 'Xem Giá' : 'Đặt Cọc'}</td>
        <td className="px-2 py-2">{price}</td>
        <td className="px-2 py-2">{point}</td>
        <td className="px-2 py-2">{createAt}</td>
      </tr>
    ));
  };
    ...
    <>
      ...
      >
        <div className="px-[24px] py-[15px]">
          <h3 className="font-medium text-black text-[18px] mb-[16px]">
          ...
          <div className="flex flex-row gap-[16px] items-start">
            <div
              ...
              className="mt-[30px] relative overflow-x-auto shadow-md sm:rounded-lg flex-1"
            >
              <table className="w-full text-sm text-left text-gray-500 ">
                <thead className="text-xs text-gray-700 uppercase bg-gray-50  ">
                  <tr>{rendersColumn(columnHistories)}</tr>
                </thead>
                <tbody>{renderHistories()}</tbody>
              </table>
            </div>
            <div
              style={{ height: 'calc(100vh - 340px)' }}
            ...
            >
              <table className="w-full text-sm text-left text-gray-500 ">
                <thead className="text-xs text-gray-700 uppercase bg-gray-50  ">
                  <tr>{rendersColumn(columnHistoriesDetail)}</tr>
                </thead>
                <tbody>{renderHistoriesDetail()}</tbody>
              </table>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-row">
        <nav aria-label="" className="flex flex-1">
